...
export default async function CreatePRPage() {
  ...
  // Fetch materials and plants
  const { data: materials } = await supabase.from('material_master').select('*').eq('is_active', true)
  const { data: plants } = await supabase.from('plant_master').select('*').eq('is_active', true)
  const { data: mappings } = await supabase.from('plant_material_mapping').select('*').eq('is_active', true)
